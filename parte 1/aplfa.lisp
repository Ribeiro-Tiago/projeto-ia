...
(defun alphaBeta(no mDepth jogador &optional (alfa 0) (beta 0))
    (cond 
        ((= 0 mDepth) (list no (aval no jogador)) )
        ( (= 1 jogador)
           (alphaBetaS (sort (mapcar  #'(lambda (board) 
             (alphaBeta board (- mDepth 1) (changePlayer jogador) alfa beta)
            )
                (sucessoresMiniMax no 'dfs  jogador)
            )  'riberionaomedeixajogarL) alfa beta mDepth jogador) 
        )
        ( (= 0 jogador)
         (alphaBetaS    (sort (mapcar  #'(lambda (board) 
                (alphaBeta board (- mDepth 1) (changePlayer jogador) alfa beta)
                ...
                (sucessoresMiniMax no 'dfs jogador)
            ) 'riberionaomedeixajogarM) alfa beta mDepth jogador)
        )
    )
)
(defun alphaBetaS(sucessores alfa beta mDepth jogador)
alfa < beta nil
alfa >= beta nil
    (
     cond 
     ((> 0 mDepth) (alphaBeta (first sucessores) (- 1 mDepth) (changePlayer jogador) alfa beta))
     (t (first sucessores))
    )
    
)

(print (alphaBeta (teste2) 2 0))